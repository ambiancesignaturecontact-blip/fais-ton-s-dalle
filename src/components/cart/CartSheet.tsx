...
export function CartSheet() {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      // Une route absente renvoie du HTML : res.json() lèverait.
      const data = await res.json().catch(() => ({}));

      if (!res.ok) {
        toast.error(data?.error || `Paiement indisponible (${res.status})`);
        return;
      }
      ...
      if (data.url) {
        ...
        toast.error(data?.error || "Réponse inattendue du paiement");
      }
    } catch {
