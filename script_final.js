var M=[{"id": "l", "n": "Menu Léger", "c": "menus", "p": 6.9, "d": "Sandwich (viande + crudités + sauce)", "cs": 1, "pop": 1}, {"id": "c", "n": "Menu Classique", "c": "menus", "p": 7.9, "d": "Sandwich + boisson", "cs": 1, "pop": 1}, {"id": "g", "n": "Menu Gourmand", "c": "menus", "p": 9.9, "d": "Sandwich + boisson + dessert", "cs": 1, "pop": 1}, {"id": "t", "n": "Tiramisu", "c": "desserts", "p": 3, "d": "Fait maison", "cs": 2}, {"id": "m", "n": "Milkshake", "c": "desserts", "p": 5, "d": "Personnalisable", "cs": 3}, {"id": "co", "n": "Coca-Cola", "c": "boissons", "p": 1}, {"id": "cz", "n": "Coca Zéro", "c": "boissons", "p": 1}, {"id": "pe", "n": "Pepsi", "c": "boissons", "p": 1}, {"id": "oa", "n": "Oasis Tropical", "c": "boissons", "p": 1}, {"id": "li", "n": "Ice Tea", "c": "boissons", "p": 1}, {"id": "or", "n": "Orangina", "c": "boissons", "p": 1}, {"id": "cr", "n": "Cristaline", "c": "boissons", "p": 1}, {"id": "sp", "n": "San Pellegrino", "c": "boissons", "p": 1}];

var M=ITEMS_PLACEHOLDER;
...
document.querySelector(".hero-btn2").addEventListener("click",function(){
  if(!window.open("https://wa.me/33672044875","_blank")){
    alert("Autorise les pop-ups pour WhatsApp ou clique : https://wa.me/33672044875");
  }
});
...
function renderMenu(g,el){
  ...
  for(var i=0;i<items.length;i++){
    var x=items[i],fn=getFn(x);
    var fname=fn==="oti"?"oti()":fn==="omm"?"omm('"+x.id+"')":fn==="om"?"om('"+x.id+"')":"ai('"+x.id+"')";
...
function setMode(m){
  ...
  document.getElementById("cmd-addr").style.display=m==="livraison"?"block":"none";
  updateCartMode();
}
...
if(cmdMode==="livraison"){
  document.getElementById("mode-livraison").className="on";
  document.getElementById("cmd-addr").style.display="block";
}
...
function makeChips(items,multi){
  return items.map(function(v){
  var cls=multi?' class="multi"':'';
    return '<button'+cls+' data-k="X" data-v="'+v+'">'+v+'</button>'.replace("X",multi?"r":"x");
  }).join("");
}
...
function om(id){
  ...
  var cu_make=CU.map(function(v){return '<button data-k="cu" data-v="'+v+'">'+v+'</button>';}).join("");
  var vi_make=VI.map(function(v){return '<button data-k="v" data-v="'+v+'">'+v+'</button>';}).join("");
  var cr_make=CR.map(function(v){return '<button class="multi" data-k="r" data-v="'+v+'">'+v+'</button>';}).join("");
  var sp_make=SP.map(function(v){return '<button class="multi" data-k="p" data-v="'+v+'">'+v+'</button>';}).join("");
  var sa_make=SA.map(function(v){return '<button data-k="s" data-v="'+v+'">'+v+'</button>';}).join("");
  mb.innerHTML='<div class="cs"><h4>Cuisson</h4><div class="chips">'+cu_make+'</div></div>'+
    '<div class="cs"><h4>Viande</h4><div class="chips">'+vi_make+'</div></div>'+
    '<div class="cs"><h4>Crudités</h4><div class="chips">'+cr_make+'</div></div>'+
    '<div class="cs"><h4>Suppléments</h4><div class="chips">'+sp_make+'</div></div>'+
    '<div class="cs"><h4>Sauce</h4><div class="chips">'+sa_make+'</div></div>';
...
function oti(){
  ...
  var ti_make=TIR.map(function(v){return '<button data-k="ti" data-v="'+v+'">'+v+'</button>';}).join("");
  mb.innerHTML='<div class="cs"><h4>Parfum</h4><div class="chips">'+ti_make+'</div></div>';
...
function omm(id){
  ...
  var ms_make=MSC.map(function(v){return '<button data-k="ch" data-v="'+v+'">'+v+'</button>';}).join("");
  var cl_make=MCL.map(function(v){return '<button class="multi" data-k="cl" data-v="'+v+'">'+v+'</button>';}).join("");
  mb.innerHTML='<div class="cs"><h4>Choix</h4><div class="chips">'+ms_make+'</div></div>'+
    '<div class="cs"><h4>Coulis (+0,50€)</h4><div class="chips">'+cl_make+'</div></div>';
...
function closeModals(){
  var ids=["mod","mod-t","mod-m"];
  ids.forEach(function(id){
    var m=document.getElementById(id);
    if(m){m.classList.remove("on");}
  });
  document.body.style.overflow="";ci=null;cs={};
}
function cm(){closeModals();}
function cmt(){closeModals();}
function cmm(){closeModals();}
...
document.querySelector(".cp-ck").addEventListener("click",function(){
  ...
  if(a)cmdAddr=a.value.trim();
  localStorage.setItem("cmdAddr",cmdAddr);
...
window.matchMedia("(prefers-color-scheme:dark)").addEventListener("change",function(e){
  if(!localStorage.getItem("t"))document.documentElement.dataset.theme=e.matches?"dark":"light";
});
